...
#define inHandlerMode() __get_IPSR()
...
{ (char *)#name, (thread), (priority), (instances), ((stacksz)/sizeof(StackType_t))  }
#endif

#if configSUPPORT_STATIC_ALLOCATION
#define osThreadStaticCreate(name, thread, priority, argument, stacksz, id)  \
static __ccm StaticTask_t x##name##TaskTCB; \
static __ccm StackType_t ux##name##TaskStack[ (stacksz)/sizeof(StackType_t) ]; \
id = xTaskCreateStatic(thread, (const portCHAR * const)#name, \
        (stacksz)/sizeof(StackType_t), argument, priority, \
        ux##name##TaskStack, &x##name##TaskTCB);
#endif
...
/**
  ...
  */
osEvent *osMessageGet (osMessageQId queue_id, uint32_t millisec, osEvent *event);
