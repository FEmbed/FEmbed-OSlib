...
osThreadId osThreadCreate (osThreadDef_t *thread_def, void *argument)
{
  ...
  xTaskCreate((pdTASK_CODE)thread_def->pthread,
              (const portCHAR * const)thread_def->name,
...
osStatus osMessagePut (osMessageQId queue_id, uint32_t info, uint32_t millisec)
{
  ...
  else if(millisec == 0)    //Add by Gene
  {
    if (xQueueSend(queue_id, &info, 0) != pdTRUE) {
      return osErrorOS;
    }
  }
  else
...
* @param  queue_id  message queue ID obtained with \ref osMessageCreate.
* @param  millisec  timeout value or 0 in case of no time-out.
* @param  event information that includes status code.
...
osEvent* osMessageGet (osMessageQId queue_id, uint32_t millisec, osEvent *event)
{
  ...
  if (inHandlerMode()) {
    ...
    portEND_SWITCHING_ISR(taskWoken);
      ...
      if (xQueueReceive(queue_id, &event->value.v, (millisec>0?ticks:0)) == pdTRUE) {
      /* We have mail */
      event->status = osEventMessage;
    }
    else {
